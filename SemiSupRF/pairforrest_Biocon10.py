...
iter = 1
gamma_scores = []
couplings = [100/3, 10.0, 10/3, 1.0, 1/3, 0.1, 1/30, 0.01, 1/300, 0.001, 0.0] 
N_rand = parser.Nrand
...
for i in range(1): 
  ...
  FE_X, FE_y = feature_expansion(X_lab, y_train_lab,  random_state = N_rand - 1) #Do the feature expansion on labelled set
  ...
  scores = []
   #Select Triplets
  idx_lab = np.arange( 0, len( X_lab ))
  idx_nlab = np.arange( 0, len( X_nlab ))
  np.random.seed(N_rand - 1)
  triplets_lab = np.random.choice( a = idx_lab, size = ( round(FE_X.shape[0] / 3) , 1), replace = True)
  triplets_nlab = np.random.choice( a = idx_nlab, size = ( round(FE_X.shape[0] / 3) , 2), replace = True)
  triplets=np.concatenate((triplets_lab,triplets_nlab),axis=-1)
  ...
  for gamma in couplings: 
    ...
    for j in range(iter+1):
      ...
      y_test_preds = expanded_predict(best_rfr, X_nlab, X_lab, y_train_lab)
      accuracy.append( score( y_test_preds, y_train_nlab ) )
      ...
      for trio in triplets:
        x1, x2, x3 = X_lab[trio[0]], X_nlab[trio[1]], X_nlab[trio[2]]
        ...
        y1_y2 = np.around(best_rfr.predict( x1_x2 ).reshape(1,1),7)
        y2_y3 = np.around(best_rfr.predict( x2_x3 ).reshape(1,1),7)
        y3_y1 = np.around(best_rfr.predict( x3_x1 ).reshape(1,1),7)
